...
function renderGlyphItem(canvas, glyphIndex) {
    ...
    ctx.fillStyle = '#000';
    ctx.font = '7px sporting_grotesque';
    ...
    ctx.fillText(glyph.unicodes.map(formatUnicode), 2, cellHeight-2);
...
function onReadFile(e) {
    ...
    var file = e.getAttribute('fonturl');
    console.log(file);
    opentype.load(file);

    font = opentype.parse(file, {lowMemory:true});
    // showErrorMessage('');
    // onFontLoaded(font);
...
var fontFileName = $(".fonturl").text();
//var fontFileNames = $("#fontchoice").children().each().getAttribute('fonturl');
// document.getElementById('font-name').innerHTML = fontFileName.split('/')[1];

// var fileButton = document.getElementById('file');
// fileButton.addEventListener('change', onReadFile, false);
var fileButton = document.getElementById('fontchoice');
fileButton.addEventListener('click', function(event) {onReadFile(event.target)}, false);
